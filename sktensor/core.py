...
def nvecs(X, n, rank, do_flipsign=True, dtype=np.float, maxdot=10**10, maxmem=40*10**9):
	"""
	...
	if issparse_mat(Xn):
		...
		if Xn.shape[1] > maxdot:
			Y = slow_dot(Xn)
		else:
			Y = Xn.dot(Xn.T)
		_, U = eigsh(Y, rank, which='LM')
	# Using svd(, full_matrices=False) as opposed to eigh( Xn.dot(Xn.T) ) to save memory when Xn.dot(Xn.T) is very big
	elif (Xn.shape[0]**2*8 > maxmem):
		u,s,v = svds(Xn,k=rank)
		return flipsign(u)
	else:
		Y = Xn.dot(Xn.T)
		N = Y.shape[0]
...
def slow_dot(X):
	...
	for i in range(X.shape[0]):
		data_i = X.data[X.indptr[i]:X.indptr[i+1]]
		idx_i = X.indices[X.indptr[i]:X.indptr[i+1]]
		for j in range(i,X.shape[0]):
			data_j = X.data[X.indptr[j]:X.indptr[j+1]]
			idx_j = X.indices[X.indptr[j]:X.indptr[j+1]]
			di = data_i[np.in1d(idx_i,idx_j,assume_unique=True)]
			if len(di) > 0:
				dj = data_j[np.in1d(idx_j,idx_i,assume_unique=True)]
				y[i,j] = np.dot(di,dj)
				y[j,i] = y[i,j]
	return y
